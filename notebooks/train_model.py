...
import joblib
import numpy as np
# --- Étape 2.8 : Simulation de l'inférence (Futur Lab 3) ---
# 1. Chargement des composants sérialisés
model_loaded = joblib.load("models/model.pkl")
# Note : Assure-toi d'avoir sauvegardé tes encodeurs avec ces noms précis
le_sexe_loaded = joblib.load("models/encoder_sexe.pkl")
le_region_loaded = joblib.load("models/encoder_region.pkl")

print(f"✅ Modèle rechargé : {type(model_loaded).__name__}")
print(f"✅ Classes détectées : {list(model_loaded.classes_)}")

# 2. Données d'un nouveau patient (Centre de santé de Médina)
nouveau_patient = {
    'age': 28,
    'sexe': 'F',
    'temperature': 39.5,
...
# Encodage des valeurs catégoriques via les encodeurs rechargés
sexe_enc = le_sexe_loaded.transform([nouveau_patient['sexe']])[0]
region_enc = le_region_loaded.transform([nouveau_patient['region']])[0]

# Création du vecteur de caractéristiques (Features Vector)
features = [
    nouveau_patient['age'],
    sexe_enc,
    nouveau_patient['temperature'],
    nouveau_patient['tension_sys'],
    int(nouveau_patient['toux']),      # Conversion booléen -> 0/1
...
# On passe les features sous forme de liste de listes [[]]
